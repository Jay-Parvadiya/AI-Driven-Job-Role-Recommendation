...
from employee import main_employee
from employee import process_bar
from recruiter import main_recruiter
import time
...
if "select_user" not in st.session_state:
...
# page transformation as per user type
def main():
    if st.session_state.user=="Employee":
        main_employee()

    if st.session_state.user=="Recruiter":
        main_recruiter()
...
def check_for_employee():
    with st.form(f"SignupForm"):
        st.title("Sign Up")
        st.caption(st.session_state.select_user)
...
def mail_varify(user_mail,user_name,select_user):
    ...
    from_mail = "pateldax2601@gmail.com" # your gmail
    server.login(from_mail, "tvou ifug xacj nyha") # passward
    to_mail = user_mail
...
def add_recruiter_folder(new_username):
    path = f"Dataset/Recruiter/{new_username}"
    os.makedirs(path, exist_ok=True)
    print("Folders created successfully!")
